export default function HowitsWork({image,heading,paragraph}){
  ...
  <div
    ...
    style={{
      background: "radial-gradient(ellipse at center, rgba(182,154,239,0.4), rgba(133,81,248,0.2))",
    }}
  ></div>
  ...
  <h3 className="mt-4 text-white font-bold text-[22px] text-center z-10">{heading}</h3>

 
  <p className="text-white text-center text-[16px] z-10 w-full max-w-[389px]">{paragraph}</p>
  </div>
</div>
    )
}